...
export async function GET() {
  ...
  let res: Response;
  try {
    res = await fetch(`${API}/support/context`, { headers: { Authorization: `Bearer ${await token()}` }, cache: 'no-store' });
...
export async function PATCH(request: Request) {
  ...
  let res: Response;
  try {
    res = await fetch(`${API}/support/context`, { method: 'PATCH', headers: { Authorization: `Bearer ${await token()}`, 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  } catch { return NextResponse.json({ message: getFriendlyApiUnavailableMessage(API) }, { status: 503 }); }
  const data = await res.json().catch(() => ({ message: 'Unexpected response' }));
  return NextResponse.json(data, { status: res.status });
}
