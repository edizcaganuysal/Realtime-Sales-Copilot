...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  let res: Response;
  try {
    res = await fetch(`${API}/support/action-definitions/${id}`, { method: 'PATCH', headers: { Authorization: `Bearer ${await token()}`, 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  let res: Response;
  try {
    res = await fetch(`${API}/support/action-definitions/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${await token()}` } });
  } catch { return NextResponse.json({ message: getFriendlyApiUnavailableMessage(API) }, { status: 503 }); }
  const data = await res.json().catch(() => ({ message: 'Unexpected response' }));
  return NextResponse.json(data, { status: res.status });
}
